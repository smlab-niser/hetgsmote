...
class HAN_Agg(nn.Module):
    def __init__(self, embed_dim, heads, dropout):
        ...
        self.v_aggregate = HAN_NeighAgg(embed_dim, heads, dropout)
    ...
    def forward(self, x_list, edges_list, x_node, edge_weight_list):
        ...
        v_aggr = self.v_aggregate(x_list[2], x_node, edges_list[2][[1, 0], :], edge_weight_list[2])
...
class HAN_ConEn(nn.Module):
    def __init__(self, embed_dim, dropout):
        ...
        self.a_con = HAN_Content_Agg(embed_dim, 2, dropout)  # 2,6,2 for aminer
        self.p_con = HAN_Content_Agg(embed_dim, 6, dropout)
        self.v_con = HAN_Content_Agg(embed_dim, 2, dropout)
...
class HAN_NetEn(nn.Module):
    def __init__(self, embed_dim, heads, dropout):
        ...
        self.v_het = HAN_Agg(embed_dim, heads, dropout)
    ...
    def forward(self, x_list, data):
        ...
        x_list[2] = self.v_het(x_list, v_edges_list, x_list[2], edge_weight_list)
