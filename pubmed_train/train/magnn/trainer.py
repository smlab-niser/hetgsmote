            ...
            for k in range(r, z):   
                ...
                for p in range(10):
                    ...
                    if train_dict[k] == 'preT' or train_dict[k] == 'preO' or train_dict[k] == 'noFT':
                        decoder_g = torch.load('../pretrained_magnn/decoder_g.pth', weights_only=False)
                        decoder_d = torch.load('../pretrained_magnn/decoder_d.pth', weights_only=False)
                        decoder_c = torch.load('../pretrained_magnn/decoder_c.pth', weights_only=False)
                        decoder_s = torch.load('../pretrained_magnn/decoder_s.pth', weights_only=False)
                    else: 
