...
def train_smote(data, edge_indices, encoder1, encoder2, classifier, decoder_list, train_idx, 
    ...
    for epoch in range(args.num_epochs):
        ...
        if os_mode == 'gsm':
            x_list[0], new_labels , new_train_idx, ar_edge_indices = oversample(features = x_list[0], labels = data['m'].y, 
            ...
            x_list[0], new_labels , new_train_idx = oversample(features = x_list[0], labels = data['m'].y, 
                train_idx = train_idx, edge_indices = None, args= args, os_mode = 'gsm')
            new_edge_indices = edge_indices
            
        elif os_mode == 'up':
            x_list[0], new_labels , new_train_idx, new_edge_indices = oversample(features = x_list[0], labels = data['m'].y,  
            ...
            new_labels, new_train_idx, new_edge_indices = data['m'].y, train_idx, edge_indices
        
        if os_mode not in ['edge_sm', 'gsm', 'em_smote', 'gsm-b', 'gsm-k']: x_list = encoder2(x_list, data)
        
        outputs = classifier(x_list, new_edge_indices, edge_weight_list, edge_index_mid = data['a', 'walk', 'd'].edge_index)
        
        if os_mode == 'reweight':
            weight = x_list[0].new((data['m'].y.max().item()+1)).fill_(1)
...
def test_smote(data, edge_indices, encoder1, encoder2, classifier, decoder_list, test_idx, os_mode, train_mode, args, dataset = "Test"):
    ...
    with torch.no_grad():
        ...
        loss_cls =  F.cross_entropy(outputs[test_idx], data['m'].y[test_idx])
        if train_mode == 'preO' or train_mode == 'preT' or train_mode == 'pret'  or train_mode == 'preo': 
            loss = sum(loss_de) * args.de_weight + loss_cls
        elif train_mode == 'recon':
        ...
        # print(loss)
        acc = accuracy(outputs[test_idx].detach(), data['m'].y[test_idx].detach()) 
        ...
        auc, f1, auc_list, f1_list = evaluate_class_performance(outputs[test_idx].clone().detach(), 
                                data['m'].y[test_idx].clone().detach(), thresh_list = thresh_list, dataset = dataset, args = args)
